...
class EzAuth
{
	...
	private function _sendMail( $to, $emailType, $vars, &$errorInfo ) // For register & forgot_password
	{
		# Email Configuration
			...
			$mailConfig[ 'email_verification' ][ 'subject' ] = 'Please Verify Your Email';
			...
			$mailConfig[ 'reset_password' ][ 'subject' ] = 'Reset Your Password';
	...
	public function register( $redirectTo, $verifyEmail = false, $verificationPage = '' )
	{
		...
		# If user's email has to be validated, generate secret code
			if ( $verifyEmail ) {
				$_POST[ 'code' ] = hash( 'sha256', $_POST['username'].$_POST['email'].microtime() );//sha256 algorithm
			} else {
				$_POST[ 'verified' ] = 1;//yes
			}
	...
	public function verifyEmail( $redirectTo )
	{
	...
	public function forgotPassword( $redirectTo, $resetPasswordPage = '' )
	{
	...
	public function resetPassword( $redirectTo )
	{
		# Make sure url query - email and code does exist
			if ( empty($_GET['email']) || empty($_GET['code']) ) {
	...
	public function isLoggedIn()
	{
		# Check whether the user's session exists
			if ( !empty($_SESSION['EzAuth']) ) return true;
	...
	public function updateSession( $name, $value = null )
	{
		if ( is_array($name) ) {
			foreach ( $name as $key => $value ) {
				$_SESSION[ 'EzAuth' ][ $key ] = $value;
			}
		} elseif ( is_string($name) ) {
			$_SESSION[ 'EzAuth' ][ $name ] = $value;
		}
	}
}