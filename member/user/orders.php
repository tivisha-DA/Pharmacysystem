...
function review_order()
{
	...
	if ( $affected <0 ) {
		$flash->danger( 'Failed to send review. Please try again later!' ); return false;
	} elseif ( $affected ==0 ) {
		$flash->warning( 'No changes at this moment!' ); return false;
	}

	$flash->success( 'Your review has been sent successfully!' ); return true;
}
...
function process_form_if_it_has_been_sent()
{
	...
	# Process form
		$result = false;
		if ( $action == 'review' ) $result = review_order();
...
# Display page
	...
	$tpl->display( 'Pages/Member/User/orders.tpl' );